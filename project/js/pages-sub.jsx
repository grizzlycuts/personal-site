        ...
        <>
          ...
          <div className="lightbox-inner" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="lightbox-meta">
              <div>
                <div className="serif" style={{ fontSize: 26, color: "oklch(0.96 0.01 80)" }}>{p.title}</div>
                <div className="meta" style={{ marginTop: 6, color: "oklch(0.78 0.01 80)" }}>{p.loc} · {p.cat === "flight" ? "Flight" : "Stills"}</div>
              </div>
              <div className="meta" style={{ color: "oklch(0.78 0.01 80)", textAlign: "right" }}>
                {String(index + 1).padStart(2, "0")} / {String(photos.length).padStart(2, "0")}<br />{p.gear}
              </div>
            </div>
          </div>
        </>
          ...
          {photos.map((p) => {
            ...
            return (
              <div className="album-item reveal" key={p.id} onClick={(e) => {
                ...
                <div className="album-item-cap">
                  <span className="m">{p.loc}</span>
...
Object.assign(window, { Blog, Album, Now, Work, Contact, Links, PageHead, Lightbox });
